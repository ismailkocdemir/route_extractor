...
if __name__ == "__main__":
	...
	itin_list = [[[item] for item in itin] for itin in itin_list]

	model = PrefixSpan.train(itin_list, minSupport=0.01, maxPatternLength=20)
	result = model.freqSequences().collect()
	result.sort(key = PrefixSpan.getKey, reverse=True)
	with open('output.txt', 'w') as out:
		for fs in result:
			out.write('{}, {}'.format(fs.sequence,fs.freq))
			out.write('\n')
